# 임베디드 백엔드에서 상태·비동기·자원 정합성을 다뤘습니다

임베디드 Linux 환경에서는 웹 요청 하나의 성공 여부만으로 시스템이 정상이라고 보기 어렵습니다. 여러 클라이언트의 상태가 같아야 하고, 비동기 작업이 오래된 결과를 남기지 않아야 하며, 파일과 장치 자원의 생명주기도 함께 관리해야 했습니다.
...
이 잠금은 **기록 시점의 동시 쓰기 충돌 가능성을 줄이는 역할**로 사용했습니다. 읽기부터 수정까지 전체 흐름을 하나의 원자적 작업으로 보장한다고 확대해서 표현하지 않습니다.
