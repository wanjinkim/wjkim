...
임베디드 Linux 환경의 레거시 PHP REST API를 정리하면서 인증, 장비별 도메인 처리, 시스템 명령 실행, 예외 처리가 서로 다른 책임이라는 점에 집중했습니다.
...
실제 DB 저장소 교체까지 적용하거나 테스트한 결과는 별도로 확인해야 합니다.
...
실제 신규 장비 추가 시 수정 범위가 얼마나 줄었는지는 별도 측정이 필요합니다.
...
실제 Mock Runner 테스트 결과는 별도 확인이 필요합니다.
